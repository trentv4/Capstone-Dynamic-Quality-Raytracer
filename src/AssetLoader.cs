...
namespace DominusCore {
	public class AssetLoader {
		...
		public static Gamepack LoadGamepack(string directory) {
			...
			Dictionary<string, string> objects = ReadIni($"{directory}/objects.ini");
			try {
				...
				string[] activeObjects = objects.Get("ActiveObjects").Split(",", StringSplitOptions.TrimEntries);
				foreach (string s in activeObjects) {
					gp.UnitTypes.Add(s, new UnitType(
						objects.Get($"{s}.Name"),
						Int32.Parse(objects.Get($"{s}.MaxHealth")),
						objects.Get($"{s}.ModelFileLocation")
					));
				}
